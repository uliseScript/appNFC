...
class MainActivity : FlutterActivity() {
    ...
    // Último valor enviado desde Flutter, por defecto 0x6B 0x00
    private var ultimoValor: ByteArray = byteArrayOf(0x6B.toByte(), 0x00)
        ...
        MethodChannel(
            ...
            when (call.method) {
                ...
                "enviarDato" -> {
                    ...
                    val data = byteArrayOf(byte1, 0x00)
                    ultimoValor = data // Guardar último valor
                    enviarDato(data)
                    result.success("Dato enviado: ${data.joinToString(" ") { "0x%02X".format(it) }}")
                }
    ...
    private val gattServerCallback = object : BluetoothGattServerCallback() {
        override fun onConnectionStateChange(device: BluetoothDevice?, status: Int, newState: Int) {
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                // Reenviar inmediatamente el último dato enviado
                if (ultimoValor.isNotEmpty()) {
                    characteristic.value = ultimoValor
                    gattServer?.notifyCharacteristicChanged(connectedDevice, characteristic, false)
                    Log.i("BLE", "📤 Reenviado al reconectar: ${ultimoValor.joinToString(" ") { "0x%02X".format(it) }}")
                }
            } else if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                Log.i("BLE", "❌ Desconectado")
                connectedDevice = null
...
class MainActivity : FlutterActivity() {
    ...
    private lateinit var characteristic: BluetoothGattCharacteristic
    private var connectedDevice: BluetoothDevice? = null
    private var ultimoValor: ByteArray = byteArrayOf(0x6B.toByte(), 0x00) // Valor por defecto
    ...
    override fun configureFlutterEngine(@NonNull flutterEngine: FlutterEngine) {
        ...
        MethodChannel(
            ...
            when (call.method) {
                ...
                "enviarDato" -> {
                    val valorHex = call.arguments as? String ?: "0x6B"
                    val byte1 = try {
                        valorHex.removePrefix("0x").toInt(16).toByte()
                    } catch (e: Exception) {
                        Log.e("BLE", "❌ Formato inválido: $valorHex")
    ...
    private val gattServerCallback = object : BluetoothGattServerCallback() {
        override fun onConnectionStateChange(device: BluetoothDevice?, status: Int, newState: Int) {
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                // Enviar el último dato al reconectar
                enviarDato(ultimoValor)
            } else if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                connectedDevice = null
        ...
        override fun onCharacteristicReadRequest(
            ...
            if (characteristic?.uuid == CHARACTERISTIC_UUID) {
                ...
                Log.i(
                    "BLE",
                    "📖 Lectura solicitada. Enviado: ${
                        ultimoValor.joinToString(" ") {
                            "0x%02X".format(it)
                        }
                    }"
                )
    ...
    private fun enviarDato(data: ByteArray) {
        ...
        try {
            ...
            Log.e("BLE", "❌ Error al reproducir sonido: ${e.message}")
        }
    }
}
